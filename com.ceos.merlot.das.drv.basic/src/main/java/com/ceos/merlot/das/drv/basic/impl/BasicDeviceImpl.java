...
public class BasicDeviceImpl implements BasicDevice, DriverCallback  {
    ...
    public BasicDeviceImpl(BundleContext bc) {
        this.bc = bc;
    }
    ...
    public void ReadRequest(String index, String item, DriverCallback cb) {
        ...
        if (plcConn.isConnected()){
            ...
            ((BasicDriver) driver).putEvent(event);
        } else {
    ...
    public void WriteRequest(String scalar, String id, List<String> values, DriverCallback cb) {
        ...
        if (plcConn.isConnected()){
            ...
            requestBuilder.addItem("Write@["+LocalTime.now().format(dtf)+"]", id, items.toArray());            
